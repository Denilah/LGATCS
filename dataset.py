...
if __name__ == '__main__':
    import h5py
    input_dir='../Data/alldata/'
    # with open("../Data/alldata/use.rawcode.txt", "r") as f:  # 打开文本
    #     data = f.readline()  # 读取文本
    #     print(data)
    # train_set=Dataset(input_dir, 'test.name.h5', 6, 'test.apiseq.h5', 20, 'test.tokens.h5', 30, 'test.desc.h5', 30)
    # train_data_loader=torch.utils.data.DataLoader(dataset=train_set, batch_size=1, shuffle=False, num_workers=1)
    # use_set=Dataset(input_dir, 'use.name.h5', 6, 'use.apiseq.h5', 20, 'use.tokens.h5', 30)
