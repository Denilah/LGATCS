...
def train(model, training_data, validation_data, optimizer, args, device):
    ...
    def save_model(model, path):
        if not os.path.exists(path):
            os.mkdir(path)
        path = path + 'joint_embed_model.h5'
        torch.save(model.state_dict(), path)

    f = open('result.csv','a',encoding='utf-8',newline='')
    csv_writer = csv.writer(f)
    csv_writer.writerow(["Epoch","re","acc","mrr","map","ndcg"])
    ...
    for epoch in range(args.Epoch):
        ...
        logger.info("Validating.")
        re, acc, mrr, map, ndcg = validate(validation_data, model, 10, config['sim_measure'])
        csv_writer.writerow([epoch+1,re,acc,mrr,map,ndcg])

        valid_mrr = mrr
        if best_mrr < valid_mrr:
            best_mrr = valid_mrr
            print("The current best mrr score is: ", best_mrr)
            # path = args.model_path + 'joint_embed_model_test.h5'
